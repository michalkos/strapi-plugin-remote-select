import type { CustomFieldOption } from "@strapi/helper-plugin";
import { CustomFieldOptions } from "@strapi/helper-plugin/dist/features/CustomFields";
import type { TranslationMessage } from "@strapi/helper-plugin/dist/types";
import * as yup from "yup";
import getTrad from "./getTrad";

type SelectType = "base" | "searchable";

const translationsOptions: Record<
  string,
  Record<SelectType, TranslationMessage>
> = {
  sourceUrlDescription: {
    ...
    searchable: {
      ...
      defaultMessage:
        'The string "{q}" in the url will be replaced with the search phrase',
    },
  },
  ...
  multiModeDescription: {
    base: {
      ...
      defaultMessage:
        "Multi mode will have a JSON array of strings in the model",
    },
    searchable: {
      id: getTrad("advanced.searchable-multi-note"),
      defaultMessage:
        "Multi mode will have a JSON array of objects in the model",
    },
  },
};

function getTranslationBySelectType(
  translationConfig: Record<SelectType, TranslationMessage>,
  type: SelectType,
): TranslationMessage {
  return translationConfig[type];
}

export function getRemoteSelectRegisterOptions(
  type: SelectType,
): CustomFieldOptions {
  return {
    base: [
      {
        ...
        items: [
          {
            name: "options.fetch.url" as any as CustomFieldOption["name"],
            type: "string",
            intlLabel: {
              id: getTrad("basic.source-url-label"),
              defaultMessage: "Fetch options url",
            },
            description: getTranslationBySelectType(
              translationsOptions.sourceUrlDescription,
              type,
            ),
          },
          {
            name: "options.fetch.method" as any as CustomFieldOption["name"],
            type: "select",
            defaultValue: "GET",
            intlLabel: {
          ...
          {
            name: "options.fetch.body" as any as CustomFieldOption["name"],
            ...
            description: getTranslationBySelectType(
              translationsOptions.fetchBodyDescription,
              type,
            ),
          },
          {
            name: "options.fetch.headers" as any as CustomFieldOption["name"],
            type: "textarea" as any,
            intlLabel: {
      ...
      },
      {
        ...
        items: [
          {
            name: "options.mapping.sourceJsonPath" as any as CustomFieldOption["name"],
            type: "string",
            defaultValue: "$",
            intlLabel: {
              id: getTrad("basic.source-url-label"),
          ...
          {
            name: "options.mapping.labelJsonPath" as any as CustomFieldOption["name"],
            type: "string",
            defaultValue: "$",
            intlLabel: {
          ...
          {
            name: "options.mapping.valueJsonPath" as any as CustomFieldOption["name"],
            type: "string",
            defaultValue: "$",
            intlLabel: {
    ...
    advanced: [
      ...
      },
      {
        ...
        items: [
          {
            name: "options.select.multi" as any as CustomFieldOption["name"],
            type: "checkbox",
            intlLabel: {
